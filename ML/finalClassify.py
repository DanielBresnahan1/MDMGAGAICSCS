...
Script with no functions that will generate a csv from the ML model predictions.
Csv will be of the form 'Image','PredictionA','PredictionB','PredictionC','True_Label'
where PredictionA is the predicted label from classifying the heat map, Generated by Model A, with Model T, 
and so on. And True_Label is 1 if the image contains a lession, and 0 if otherwise. 
