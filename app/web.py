...
def getProModel():
    ...
    trueLabel : float
        The actual label data.
    """
    url = "https://mdmgcapstone.s3.amazonaws.com/Train_Maps/"
    x = 0
    y = 0
    z = 0
    mapA = 0
    ...
    trueLabel = 0
    stuff = []
    with open('./app/classifications.csv', newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            x = row[1]
            y = row[2]
            z = row[3]
            mapA = row[0] + "/modelA.png"
            mapB = row[0] + "/modelB.png"
            mapC = row[0] + "/modelC.png"
            label = row[4]
            trueLabel = row[5]
            ...
            imgB = url + mapB
            imgC = url + mapC
            stuff.append([row[0], x, y, z, imgA, imgB, imgC, label, trueLabel])
    return stuff
...
def pro_model():
    ...
    """
    stuff = getProModel()
    return render_template('pro_model.html', stuff=stuff)
